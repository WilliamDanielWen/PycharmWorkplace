...
class Crawler(object):
    ...
    def __init__(self
        ...
        self.request_queue_lock=requestQueueLock

        self.selection_queue = selectionQueue # type multiprocess.Queue
        self.selection_queue_lock=selectionQueueLock
    ...
    def start(self):
        ...
        process_pool=set()
        for i in range(0,self.num_requestProcess,1):
            crawl_process = Process(target=self.process_requestURL_andStore)
            crawl_process.daemon = True
            crawl_process.start()
            process_pool.add(crawl_process)

        while True:
            time.sleep(5)
            print "Request-Queue size is :",self.request_queue.qsize()
            for p in process_pool:
                if not p.is_alive():
                    ...
                    new_process = Process(target=self.process_requestURL_andStore)
    ...
    def process_requestURL_andStore(self):
        #print "New Process Created, id ",os.getpid()


        try:
            requestURL_info = self.request_queue.get(block=False)
        except Exception,e:
            print "Crawler: request_queue fail to read exception"
            return
        ...
        try:
            ...
            print 'Crawler Timeout request exception:',e.message," Ignore ",request_url
            return
        ...
        try:
            ...
            if status['created']:
                ...
                print '## Crawler Store Success : ##', request_url
                print self.num_stored_page.value,'   page stored in ',timeUsed,' seconds, with ', self.num_requested_page.value," requests made"
                return
