...
#loop on
def get_fc7_feature(images, layer ='fc7'):
...
# Initialize files
import h5py
f = h5py.File(data_folder_location + 'first_sampled_data_image_fc7features.h5', 'w')
filenames = f.create_dataset('photo_id',(0,), maxshape=(None,),dtype='|S54')
feature = f.create_dataset('feature',(0,4096), maxshape = (None,4096))
f.close()

import pandas as pd
photos_indexes = pd.read_csv(data_folder_location + 'first_sampled_data_photo_to_biz_ids.csv')
photo_location = data_folder_location + 'train_photos/'
image_set = [os.path.join(photo_location, str(x) + '.jpg') for x in photos_indexes['photo_id']]  # get full filename
...
for i in range(0, num_train, batch_size):
    images = image_set[i: min(i + batch_size, num_train)]
    features = get_fc7_feature(images, layer='fc7')
    num_done = i+features.shape[0]
    f= h5py.File(data_folder_location + 'first_sampled_data_image_fc7features.h5', 'r+')
